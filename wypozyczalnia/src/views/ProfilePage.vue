<template>
    <v-container fluid fill>
        <v-toolbar flat class="transparent">
            <v-toolbar-title v-if="!ownProfile" class="display-1">Profil użytkownika {{user.firstName}}
                {{user.lastName}}
            </v-toolbar-title>
        ...
        <v-toolbar flat class="transparent">
            ...
            <v-spacer/>
            <v-btn icon @click="dialogNames=true" color="info" fab small>
                <v-icon>edit</v-icon>
            </v-btn>
        </v-toolbar>
        <v-list style="margin: 10px">
            <v-list-tile>
                <v-list-tile-content>
                    <v-list-tile-title>
                        <span class="title">Imię: </span> <span
                            class="title font-weight-light">{{user.firstName}}</span>
                    </v-list-tile-title>
                </v-list-tile-content>
            </v-list-tile>
            <v-list-tile>
                <v-list-tile-content>
                    <v-list-tile-title>
                        <span class="title">Nazwisko: </span> <span
                            class="title font-weight-light">{{user.lastName}}</span>
                    </v-list-tile-title>
                </v-list-tile-content>
            </v-list-tile>
        </v-list>
        <div v-if="hasAdminRights">
            ...
            <v-list style="margin: 10px">
                <v-list-tile>
                    ...
                    <v-list-tile-action>
                        <v-btn icon @click="dialogRole=true" color="info" fab small>
                            <v-icon>edit</v-icon>
                        </v-btn>
                    </v-list-tile-action>
                </v-list-tile>
        ...
        <v-toolbar flat class="transparent">
            ...
            <v-btn v-if="hasAdminRights&&!ownProfile" @click="deleteConfirm = true" outline color="error">Usuń
                użytkownika
            </v-btn>
        </v-toolbar>
            ...
            <v-card style="max-width: 500px;">
                <v-card-text>
                    <v-form ref="nameForm" v-model="nameForm">
                        <v-text-field :rules="[rules.required]" v-model="firstName" label="Imię"
                                      required></v-text-field>
                        <v-text-field :rules="[rules.required]" v-model="lastName" label="Nazwisko"
                                      required></v-text-field>
                    </v-form>
                </v-card-text>
                <v-card-actions>
            ...
            <v-card style="max-width: 500px;">
                <v-card-text>
                    <v-form ref="passwordForm" v-model="passwordForm">
                        <v-text-field type="password" :rules="[rules.required, rules.min]" v-model="password"
                                      label="Hasło" required></v-text-field>
                        <v-text-field type="password" :rules="[rules.matching, rules.required, rules.min]"
                                      v-model="passwordConfirm" label="Powtórz hasło" required></v-text-field>
                    </v-form>
                    <div class="caption font-weight-light error--text" v-if="passErr">Błąd. Nie udało się zmienić
                        hasła.
                    </div>
                </v-card-text>
                <v-card-actions>
        ...
        <v-dialog v-model="deleteConfirm" max-width="500" style="align-content: center">
            <v-card style="max-width: 500px;">
                ...
                <v-card-actions>
                    ...
                    <v-btn outline @click="deleteConfirm=false">Nie</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-container>
        ...
        methods: {
            ...
            async changePassword() {
                ...
                if (this.passwordForm) {
                    try {
                        this.response = await axios.put('/users/change/password', {
                            "Password": this.password,
                            "RepeatedPassword": this.passwordConfirm
                        });
            ...
            async changeNames() {
                ...
                if ((this.firstName !== this.user.firstName) || (this.lastName !== this.user.lastName)) {
                    if (this.nameForm) {
                        try {
                            this.response = await axios.put('/users/' + this.user.userId, {
